...
class IResNet(nn.Module):
    ...
    def calculate_serfiq(self, 
        ...
        if T < 2:
            raise ValueError(f"SER-FIQ parameter T is {T}, but needs to be T>1")
        
        train_mode = self.dropout.training
        self.dropout.train()
        ...
        x = self.bn2(x)
        x = torch.flatten(x, 1)
        x = x.repeat_interleave(T,dim=0)
        x = self.dropout(x)
        x = self.fc(x)
        x = self.features(x)
        
        norm = torch.linalg.vector_norm(x,ord=2,dim=1).unsqueeze(dim=1)
        x = x / norm
        
        scores = torch.empty((batch_size,), device=x.device)
        ...
        for i in range(batch_size):
            ...
            mean    = torch.triu(dist).mean()
            scores[i] = 2*(1/(1+torch.exp(scaling*mean)))
